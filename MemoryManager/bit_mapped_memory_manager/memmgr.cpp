...
// 每个大的内存块BitMapEntry被分成了INT_SIZE个小块，每个小块有INT_SIZE个内存小块
...
Complex* BitMapEntry::ComplexObjectAddress(int pos) {
	SetBit(pos, false);
	//(Head()) + (pos / INT_SIZE))确定位于BIT_MAP_ELEMENTS中哪一个块，然后再确定具体的小Complex内存块
...
void* MemoryManager::allocate(size_t size) {
	if (size == sizeof(Complex))    // non-array version
	{
...
// 申请大小为BIT_MAP_SIZE*sizeof(Complex)的内存块，并保存
